// Helper pour gérer les assets en production
// Les imports figma:asset ne fonctionnent qu'en développement Figma Make

interface AssetMap {
  [key: string]: string;
}

// Mapping des assets Figma vers des URLs publiques ou fallback
// En production, utilisez vos propres URLs d'images
const assetMapping: AssetMap = {
  // Profile image - PHOTO PRINCIPALE
  ...
  // Project images - SCREENSHOTS DE PROJETS
  ...
  // Avatar images - AVATARS TESTIMONIALS
  ...
  // Tech stack logos - LOGOS TECHNOLOGIES
...
/**
 * Résout un import figma:asset vers une URL utilisable en production
 * @param assetImport - L'import depuis figma:asset ou une URL directe
 * @returns URL de l'image utilisable
 */
export function resolveAsset(assetImport: string): string {
  // Si c'est déjà une URL complète, la retourner directement
  if (assetImport.startsWith('http://') || assetImport.startsWith('https://')) {
    return assetImport;
  }
  ...
  // Extraire le hash du nom de fichier
  const hashMatch = assetImport.match(/([a-f0-9]{40})\.(png|jpg|jpeg|svg)/);
  if (hashMatch) {
    const hash = hashMatch[0];
    return assetMapping[hash] || assetImport;
  }
  
  // Retourner l'import original si aucun mapping trouvé
  return assetImport;
}

/**
 * Vérifie si on est en environnement de développement Figma Make
 */
export function isFigmaDevEnvironment(): boolean {
